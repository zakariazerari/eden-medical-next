...
export default function DriversPage() {
  ...
  const fetchDrivers = async () => {
    ...
    try {
      // ✅ Request drivers WITH images for admin
      const res = await fetch("/api/drivers?includeImages=true", { 
        cache: 'no-store'
      });
  ...
  return (
    <div className="p-6 md:ml-64 space-y-8 bg-gray-50">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-4xl font-extrabold text-gray-800">Driver Management</h1>
          <p className="text-gray-600 mt-2">Add drivers - Customers will review them on the website</p>
        </div>
        <button
          onClick={() => {
            resetForm();
          ...
          className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all font-semibold flex items-center gap-2"
        >
      ...
      {showForm && (
        <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8 border-2 border-gray-200">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            {/* Image Upload */}
            <div className="flex flex-col items-center gap-4 p-6 bg-gray-100 rounded-xl border-2 border-dashed border-gray-300">
              <div className="relative">
                {imagePreview ? (
                  ...
                  <div className="w-32 h-32 rounded-full bg-gradient-to-br from-gray-500 to-gray-600 flex items-center justify-center">
                    <FaUserCircle className="text-7xl text-white" />
                  </div>
                )}
              </div>
              
              <label className="cursor-pointer bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl font-semibold flex items-center gap-2 transition-all">
                <FaImage />
            ...
            <div className="flex gap-4">
              <button
                type="submit"
                disabled={uploading}
                className={`bg-gradient-to-r from-blue-600 to-blue-700 text-white px-8 py-3 rounded-xl font-semibold hover:shadow-lg transition-all ${uploading ? 'opacity-50' : ''}`}
              >
      ...
      <div className="bg-white rounded-2xl shadow-xl p-6 md:p-8">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">All Drivers ({drivers.length})</h2>
        ...
        {drivers.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-lg text-gray-500">No drivers added yet</p>
          </div>
        ) : (
          <div className="space-y-4">
            {drivers.map((driver) => (
              <div
                key={driver._id}
                className={`border-2 rounded-xl p-6 ${driver.isActive ? 'border-blue-200 bg-blue-50' : 'border-gray-200 bg-gray-50 opacity-60'}`}
              >
                <div className="flex flex-col md:flex-row gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-600 mb-2">Age: {driver.age} years</p>
                    
                    <div className="flex items-center gap-3 mb-2">
                      <div className="flex gap-1">
                        {[...Array(5)].map((_, i) => (
                          <FaStar key={i} className={i < Math.round(driver.averageRating) ? "text-red-500" : "text-gray-300"} />
                        ))}
                      </div>
                      <span className="font-bold text-gray-800">{driver.averageRating || 0}</span>
                      <span className="text-sm text-gray-600">({driver.totalReviews || 0} reviews)</span>
                    </div>
                  ...
                  <div className="flex md:flex-col gap-2">
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg font-semibold flex items-center gap-2 ${driver.isActive ? 'bg-blue-100 text-blue-700' : 'bg-gray-200 text-gray-700'}`}
                    >
